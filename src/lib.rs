...
pub fn encrypt(t: String, password: String) -> String
{
  ...
  let mut _key = [0u8; 16];
  hasher.finalize_variable(|out| {_key.copy_from_slice(out);});
...
pub fn get_password() -> String
{
  ...
  let mut cursor = connection.prepare("SELECT password FROM databases WHERE [default] = 1 ").unwrap().cursor();
...
pub fn get_save_password_by_id(id: &String) -> String
{
  ...
  let connection = sqlite::open("settings.db").unwrap();
  let sql = format!("SELECT save_password FROM databases WHERE id = {} LIMIT 1", id );
  let mut cursor = connection.prepare(sql).unwrap().cursor();
  while let Some(row) = cursor.next().unwrap()
  {
...
pub fn get_databases() -> ListStore
{
  ...
  while let Some(row) = cursor.next().unwrap()
  {
    model.insert_with_values(None, &[ 0, 1], &[ &row[0].as_string().unwrap() , &row[1].as_string().unwrap()    ] );      
  }
  model
}
...
pub fn create_and_fill_model_cat() -> ListStore
{
  ...
  let mut cursor = connection.prepare("SELECT name, cast(id as text) FROM categories ORDER BY sort, name ").unwrap().cursor();
  while let Some(row) = cursor.next().unwrap()
  {
    model.insert_with_values(None, &[ 0, 1], &[ &row[0].as_string().unwrap() , &row[1].as_string().unwrap()    ] );      
  }
...
pub fn get_subcategory_by_id(id: &String)->String
{
  ...
  let connection = sqlite::open("databases/database.db").unwrap();
  let sql = format!("SELECT content FROM subcategories WHERE id = {} LIMIT 1", id );
  let mut cursor = connection.prepare(sql).unwrap().cursor();
  while let Some(row) = cursor.next().unwrap()
  {
...
pub fn append_column(tree: &TreeView, id: i32, title: &'static str, store: &ListStore, cat_id: &str)
{
  ...
  cell.connect_edited(clone!(@weak store => move |_cellrenderertext, path, new_text|
  {
...
pub fn update_category_name(new_name: String, id: String )
{
   ...
   let connection = sqlite::open("databases/database.db").unwrap();
   let sql = format!("SELECT name, cast(id as text) id FROM categories ORDER BY sort, name LIMIT {},1", id );
   let mut cursor = connection.prepare(sql).unwrap().cursor();
   while let Some(row) = cursor.next().unwrap()
   {
   ...
   let sql2 = format!("UPDATE categories SET name = '{0}' WHERE id = {1} ", new_name, _id );
...
pub fn update_subcategory_name(new_name: String, id: String, cat_id: &str)
{
   ...
   let re = Regex::new(r"[^a-zA-Z\d\s:_-]").unwrap();
   let clean_name = re.replace_all(&new_name, "");
   let sql2 = format!("UPDATE subcategories SET name = '{0}' WHERE id = {1}", clean_name, _id );
   println!("{:?}",sql2);   
...
pub fn delete_subcategory(id: &String)->String
{
   ...
   let connection = sqlite::open("databases/database.db").unwrap();
   let sql = format!("SELECT cast(category_id as TEXT) catetgory_id FROM subcategories WHERE id = {0} LIMIT 1", id );
   //println!("{:?}", sql);
...
pub fn add_subcategory(id: &String, textview: &String)
{
  ...
  let connection = sqlite::open("databases/database.db").unwrap();
  let sql = format!("INSERT INTO subcategories(name,category_id,content)VALUES('',{0},'{1}')", id, textview.to_string());
  println!("{:?}",sql);
  let _r = connection.execute(&sql);
}
...
pub fn update_subcategory(id: &String, textview: &String)
{
  println!("add subcat row {:?}", id);
  let connection = sqlite::open("databases/database.db").unwrap();
  let sql = format!("UPDATE subcategories SET content = '{0}' WHERE id = {1}", textview.to_string(), id);
  println!("{:?}",sql);
  let _r = connection.execute(&sql);
}
